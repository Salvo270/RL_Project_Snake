"""
PPO Agent - Comprehensive Evaluation Suite.
Mirrors the fully/partially observable DQN evaluation methodology for fair comparison.
"""
...
class EvaluationConfig:
    ...
    greedy: bool = True
    save_gif: bool = True
    gif_filename: str = "best_ppo_agent_gameplay.gif"
    summary_json: str = "evaluation_ppo_summary.json"
    episodes_csv: str = "evaluation_ppo_episodes.csv"


def get_env(n: int = 1, size: int = 7, mask_size: int = 2):
    return environments_partially_observable.OriginalSnakeEnvironment(n, size, mask_size)
...
def evaluate_one_episode(
    ...
    for _ in range(max_steps):
        ...
        reward_tensor = env.move(action_array)
...
def build_agent_for_eval(
    ...
    env_dummy = get_env(1, board_size, mask_size)
    raw_state_shape = tuple(np.asarray(env_dummy.to_state()).shape[1:])
    
    config = PPOConfig(epsilon_start=0.0, epsilon_end=0.0)
    agent = PPOAgent(
    ...
    dummy = np.zeros((1, *agent.stacked_shape), dtype=np.float32)
    _ = agent.model(dummy, training=False)
...
def run_evaluation(weights_path: str, cfg: EvaluationConfig) -> Dict[str, object]:
    agent = build_agent_for_eval(weights_path, cfg.board_size, cfg.mask_size, cfg.n_actions)

    all_rows: List[Dict[str, float | int]] = []
    best_reward = -np.inf
    best_frames: List[np.ndarray] = []

    print("\nStarting evaluation (PPO Agent)...")
    print(
    ...
    for seed in cfg.seeds:
        ...
        for episode_idx in range(cfg.n_episodes_per_seed):
            env = get_env(n=1, size=cfg.board_size, mask_size=cfg.mask_size)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate a trained PPO Snake agent")
